...
class CollectedDataset:
    ...
    def generate_blank_data(self, sample, masked_lengths, attention_mask, rng, task='bert'):
        ...
        # print("generate_blank_data", tokens[:10])
    ...
    def split_samples(self, samples, rng):
        ...
        for sample in samples:
            tokens, loss_masks = sample['input_ids'][1:], sample['loss_mask'][1:]
            ...
            for _ in range(num_splits):
                ...
                new_samples.append({'input_ids': new_tokens, 'loss_mask': new_loss_masks})
        return new_samples
    ...
    def construct_blocks(self, samples):
        ...
        if rand < self.bert_prob:
            ...
            for sample in samples:
                ...
                if single_span:
                    ...
                    while masked_count < int(self.bert_ratio * len(sample['input_ids'])):
                ...
                if self.masked_lm:
                    ...
                    sep = len(sample['input_ids']) - masked_count + len(masked_lengths)
            ...
            max_generation_length = rng.randint(
                int(self.gpt_min_ratio * min(map(lambda x: len(x['input_ids']), samples))),
                max(map(lambda x: len(x['input_ids']), samples)) - 2)
            for sample in samples:
                generation_length = min(max_generation_length, len(sample['input_ids']) - 2)
                attention_mask.append(len(sample['input_ids']) - generation_length + 1)
                multiple_doc = index_in_list(sample['input_ids'], self.tokenizer.get_command('eos').Id) not in [-1, len(
                    sample['input_ids']) - 1]
        ...
        if self.encoder_decoder:
            ...
            token_batch, target_batch, loss_mask_batch, position_id_batch = self.pad_batch(token_batch, target_batch,
                                                                                           loss_mask_batch,
                                                                                           position_id_batch)
            return {'input_ids': torch.tensor(token_batch, dtype=torch.long),
                    'target': torch.tensor(target_batch, dtype=torch.long),
                    'loss_mask': torch.tensor(loss_mask_batch, dtype=torch.long),
